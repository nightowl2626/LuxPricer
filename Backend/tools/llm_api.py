...
def query_llm(prompt: str, client=None, model=None, provider="openai", image_path: Optional[str] = None, image_paths: Optional[List[str]] = None) -> Optional[str]:
    """
    Query an LLM with a prompt and optional image attachment(s).
    ...
    Args:
        ...
        image_path (str, optional): Path to a single image file to attach
        image_paths (List[str], optional): Paths to multiple image files to attach
    ...
    # Prioritize image_paths over image_path if both are provided
    if image_paths and image_path:
        print(f"Warning: Both image_path and image_paths provided, using image_paths", file=sys.stderr)
    
    # If image_path is provided but image_paths is not, convert to list format
    ...
    try:
        ...
        if provider in ["openai", "local", "deepseek", "azure", "siliconflow"]:
            ...
            if image_paths:
                ...
                if provider == "openai":
                    ...
                    for img_path in image_paths:
                        encoded_image, mime_type = encode_image_file(img_path)
                        messages[0]["content"].append(
                            {"type": "image_url", "image_url": {"url": f"data:{mime_type};base64,{encoded_image}"}}
                        )
                else:
                    # Other providers may not support multiple images
                    print(f"Warning: Multiple image support not confirmed for provider {provider}", file=sys.stderr)
                    # Use only the first image
                    encoded_image, mime_type = encode_image_file(image_paths[0])
                    messages[0]["content"] = [
            ...
            if image_paths:
                # Gemini can handle multiple images in a single message
                parts = [prompt]
                for img_path in image_paths:
                    file = genai.upload_file(img_path, mime_type="image/png")
                    parts.append(file)
...
def main():
    ...
    parser.add_argument('--image', type=str, help='Path to a single image file to attach to the prompt')
    parser.add_argument('--images', nargs='+', type=str, help='Paths to multiple image files to attach to the prompt')
    ...
    # Handle both single and multiple image modes
    if args.images:
        response = query_llm(args.prompt, client, model=args.model, provider=args.provider, image_paths=args.images)
    else:
